...
outpath = f'./data/medmentions_vicuna13b_prompt_variants_by_source_1000.json'
...
def main():
    ...
    for i, d in enumerate(tqdm(prompt_variant_results[:sample_size])):
        ...
        for output_type, output_func, output_type_template in output_types:
            options = d['options']
            for source_limit in ['all', 'metamap', 'quickumls', 'bm25']:
                if source_limit != 'all':
...
def call_vicuna(input, n_predict=128, temperature=0.2):
    response = ollama.chat(model='vicuna:13b', messages=[
        {
            'role': 'user',
            'content': input,
        },
