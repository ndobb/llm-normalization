...
outpath = f'./data/medmentions_vicuna13b_prompt_variants_by_source.json'
...
def main():
    ...
    for i, d in enumerate(tqdm(prompt_variant_results[:sample_size])):
        ...
        for output_type, output_func, output_type_template in output_types:
            options = d['options']
            for specific_source in ['all', 'metamap', 'bm25', 'quickumls']:
                if specific_source != 'all':
                    options = [o for o in options if specific_source in o['sources']]
                for use_CoT in [True]:
                    for max_answers in [2]:
                        out_type = output_type + '_' + str(max_answers) + '_' + specific_source
                        if use_CoT:
                            out_type += '_CoT'
