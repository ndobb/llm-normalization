...
data_path = os.path.join('./data', 'medmentions_vicuna13b_prompt_variants_yesno_1000.json')
...
def main():
    ...
    for i, d in enumerate(prompt_variant_results[:sample_size]):
        ...
        if 'responses' in d:
            ...
            for use_CoT in [True,False]:
                ...
                for j, o in enumerate(options):
                    if not any([x for x in d['responses'][out_type]['responses'] if x['answer'] == o['cui']]):
                        text = return_str(o, 0) + ' ' + return_sty(o)
                        full_prompt = f'{system_prompt.replace("{concept}", text).replace("{{term}}", "{{" + d["text"] + "}}")}'

                        print(f'Document {i} option {j} calling Vicuna...')
